...
import {
  ...
  shouldMediumRecipeCompletionRail,
...
export function MissionAwareHomeProvider({
  ...
  useEffect(() => {
    ...
    /** Infer/session: refetch immediately (no 350ms debounce). Coalesce bursts in one turn so we do not stack duplicate GETs. */
...
/** Sections area: full recipe takeover only at high confidence; medium gets a compact rail below merchandising. */
